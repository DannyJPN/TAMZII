    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try
        {
            ...
            for(int i =0;i<levels.size();i++)
            {
                ...
                Log.d("Level MAXLEN :",String.valueOf(i+1));
                Log.d("Level MAXLEN linelen:",String.valueOf(maxlen));

                for(int j = 0;j<levels.get(i).size();j++)
                {
                    StringBuilder mid = new StringBuilder(levels.get(i).get(j));
                        while(maxlen >mid.length()) {
                             mid.append("0");
                            levels.get(i).set(j,mid.toString());
                        }
                    Log.d("Level MAXLEN line:", levels.get(i).get(j));
